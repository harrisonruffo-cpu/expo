...
- [iOS] Build a prebuilt Expo module from source when its XCFramework binds `ExpoModulesCore` symbols that the installed `expo-modules-core` does not export; compatible modules keep their prebuilt xcframeworks. Because Expo packages publish independently, a project can resolve a module built against a different core; linking them builds with no error, then crashes at launch with dyld `Symbol not found` and is rejected on upload as ITMS-90863. Symbols are compared rather than versions, since core both adds and removes exports between patches. Based on the approach in [#46881](https://github.com/expo/expo/pull/46881) by [@rohan-patra](https://github.com/rohan-patra).
